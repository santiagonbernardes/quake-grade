...
st.info("Mergulhe nos dados dos terremotos que cercam o México. Esta ferramenta analítica permite entender a intensidade dos tremores e prever sua gravidade com base em características fundamentais.")
st.write("")


# Define colunas esperadas do dataset
colunas_esperadas = ['Magnitud',	'Latitud',	'Longitud',	'Profundidad']  # Ajuste para seu dataset
...
if not st.session_state.get('upload_valido', True):
    ...
    st.info(f"O dataset enviado não possui as seguintes colunas obrigatórias: {faltando}. Por favor, envie um arquivo no formato correto.")
...
if st.session_state['df'] is not None and st.session_state.get('upload_valido', True):
    ...
    opcao = st.sidebar.radio("Escolha a funcionalidade:", ["Análise Descritiva", "Análise Preditiva de Gravidade"])
    ...
    if opcao == "Análise Descritiva":
        ...
        with col1:
            ...
            col_num = st.selectbox("Selecione coluna numérica", options=df.select_dtypes(include=np.number).columns)
            if col_num:
                fig, ax = plt.subplots()
        ...
        with col2:
            ...
            col_num_box = st.selectbox("Selecione coluna numérica", options=df.select_dtypes(include=np.number).columns, key='box')
            if col_num_box:
                fig, ax = plt.subplots()
